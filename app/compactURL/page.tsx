        ...
        <main className={`flex z-10 justify-center items-center h-screen ${animationClass} relative overflow-hidden `}>
          ...
          <div className="container m-2 w-full bg-amber-300 h-1/2 content-center text-center p-4 rounded-md flex flex-col justify-evenly sm:w-1/2 shadow-lg shadow-neutral-400">
            ...
            <Link target="" href={`${data.shortURL}`} className="hover:bg-amber-700 rounded-md bg-amber-500 cursor-pointer transition-all ease-in-out duration-300 hover:text-white text-black">
              {data.URL ? <div className="w-full h-full p-2 flex flex-col sm:flex-row justify-evenly items-center"> <span>{data.message}</span> <div className="font-bold text-md">{data.shortURL.toUpperCase()} </div> <span className="text-xs">{data.URL}</span> {<button onClick={(e) => copyToClipboard({ e, text: (window.location.origin + "/" + data.shortURL) })}><LiaCopy size={25}  /></button>}</div> : <div className="w-full h-full p-2 cursor-not-allowed">Generate URL</div>} 
            </Link>
